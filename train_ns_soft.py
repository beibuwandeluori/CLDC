...
from losses.losses import LabelSmoothing, FocalLoss, MyCrossEntropyLoss
...
def train_model(epoch):
    ...
    for i, (XI, label, soft_label) in enumerate(train_loader):
        x = Variable(XI.cuda(device_id))
        # label = Variable(torch.LongTensor(label).cuda(device_id))
        label = Variable(label.cuda(device_id))
        soft_label = Variable(soft_label.cuda(device_id))
        ...
        output = model(x)
        # Compute and print loss
        loss = criterion(output, label) + soft_weight * MyCrossEntropyLoss()(output, soft_label)
        # update metrics
        losses.update(loss.data.item(), x.size(0))
        confs, predicts = torch.max(output.detach(), dim=1)
        acc_score.update(calculate_metrics(predicts.cpu(), label.cpu()), 1)
...
if __name__ == '__main__':
    ...
    writeFile = './output/logs/' + model_name + '_' + str(k) + '_' + str(input_size) + '_soft'
    store_name = './output/weights/' + model_name + '_' + str(k) + '_' + str(input_size) + '_soft'
    if is_alb:
        writeFile += '_alb'
        store_name += '_alb'
    ...
    if is_train:
        xdl = CLDCDataset(is_one_hot=True, transform=get_train_transforms(size=input_size, is_alb=is_alb),
                          is_alb=is_alb, k=k, do_cutmix=is_mixcut, soft_npy_path=soft_npy_path)
